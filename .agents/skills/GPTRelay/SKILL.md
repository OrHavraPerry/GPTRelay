...
These are workload hypotheses, not universal rankings. For dated prices, independent benchmark results, limitations, and sources, read [references/model-evidence.md](references/model-evidence.md) when comparing or changing routes. Keep API dollars and Codex credits separate. Refresh when availability/pricing changes or observed acceptance invalidates a route; do not browse every routine turn.
